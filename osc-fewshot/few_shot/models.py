...
class CTM(nn.Module):
    ...
    def __init__(self, n_way, n_shot, n_queries, in_channels=64, pretrained_path=None):
        ...
        self.concentrator = self._make_vgg_layer(2, in_concentrator, 40) # Input shape (NK, m1, d1, d1) -> Output shape (N, m2, d2, d2)

        in_projector = n_way * 40
        self.projector = self._make_vgg_layer(2, in_projector, 40) # Input shape (1, Nm2, d2, d2) -> Output shape (1, m3, d3, d3)

        self.reshaper = self._make_vgg_layer(2, in_channels, 40) # Output shape (NK, m3, d3, d3)
